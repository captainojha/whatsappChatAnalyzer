...
def timeline(p_num):
    ...
    dff=df.dropna()
    date_range = pd.date_range(start=str(dff["Date"][0]), end=str(dff["Date"][dff.index[-1]]))
    ...
    plt.ylabel('Messages exchanged', fontsize=16)
    plt.title('Chat Activity', fontsize=20)
